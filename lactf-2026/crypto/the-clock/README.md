...
However, the group law `clockadd` is not addition of points on an elliptic curve, instead it is actually isomorphic tocomplex multiplication. If we interpret each point $(x,y)$ as representing the complex number $y + ix$, then $(y_1 + ix_1)(y_2 + ix_2)=(y_{1}y_{2}-x_{1}x_{2}, x_{1}y_{2}+y_{1}x_{2})$.
