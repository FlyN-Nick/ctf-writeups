...
The fundamental vulnerability is that the chosen base point and all other points derived from it lie on the unit circle defined by $x^2 + y^2 = 1$. This vulnerability has two consequences. First, it allows us to easily deduce the redacted prime $p$ using gcd. Second, it causes a known group order which happens to be very smooth (it factorizes into small primes). With this known smooth group order, the discrete log problem can be reduced into multiple smaller, manageable DLPs with the **Pohlig-Hellman** attack.
...
1 & \text{if } a \equiv b^2 \pmod{p} \text{ for some } b \\
-1 & \text{if } a \not\equiv b^2 \pmod{p} \text{ for any } b
\end{cases}
$$

For a fixed $x$, the number of solutions to $y^2 = 1 - x^2$ is $1 + \chi(1-x^2)$, because if $1-x^2$ is a nonzero quadratic residue, there are 2 solutions for $y$ (one positive and one negative), if $1-x^2$ is zero, there is 1 solution for $y$, and if $1-x^2$ is a nonresidue, there are no solutions for $y$. Hence, the total number of solutions and thus the group order is $|G| = \sum\limits_{x \in \mathbb{F}_p} (1 + \chi(1-x^2)) = p + \sum\limits_{x \in \mathbb{F}_p} \chi(1-x^2)$. The sum $\sum\limits_{x \in \mathbb{F}_p} \chi(1-x^2)$ is a well-known character sum that evaluates to $-1$ if $p \equiv 1 \pmod{4}$ and $1$ if $p \equiv 3 \pmod{4}$. Thus, the group order is $|G| = p - 1$ if $p \equiv 1 \pmod{4}$ and $|G| = p + 1$ if $p \equiv 3 \pmod{4}$.
